...
pub fn get_local_ip() -> Option<Ipv4Addr>{
    for iface in get_if_addrs().unwrap() {
        ...
        */
        if !iface.is_loopback() && iface.ip().is_ipv4() && !ip.starts_with("172.17") {
...
*/
pub fn redirect_minecraft_to_a_port(mc_port: u16, wanted_port: u16) {
    ...
    match igd::search_gateway(Default::default()) {
        ...
        Ok(gateway) => {
            match gateway.add_port(igd::PortMappingProtocol::TCP, wanted_port, local_addr.into(), 60, "Minecraft client PortForward") {
                Err(ref err) => {
